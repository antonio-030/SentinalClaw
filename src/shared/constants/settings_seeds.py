"""
Seed-Definitionen für systemweite Einstellungen.

Enthält alle Standard-Werte die beim ersten Start
in die system_settings-Tabelle eingefügt werden.
Jeder Eintrag definiert Key, Default-Wert, Kategorie, Typ und UI-Label.
"""
...
class SettingDefinition(TypedDict):
    """Typisierte Struktur für eine Einstellungs-Seed-Definition."""
...
SEED_DEFINITIONS: list[SettingDefinition] = [
    # --- Tool-Timeouts ---
    ...
    {"key": "default_port_range", "value": DEFAULT_SCAN_PORT_RANGE,
     ...
     "label": "Standard-Portbereich",
     "description": "Standard-Ports wenn kein Profil gewählt"},
    # --- LLM ---
    ...
    {"key": "llm_max_tokens_per_scan", "value": str(DEFAULT_LLM_MAX_TOKENS_PER_SCAN),
     "category": "llm", "value_type": "int",
     "label": "Max Tokens pro Scan",
     "description": "Token-Budget pro Scan-Durchlauf"},
    {"key": "llm_monthly_token_limit", "value": str(DEFAULT_LLM_MONTHLY_TOKEN_LIMIT),
     "category": "llm", "value_type": "int",
     "label": "Monatliches Token-Limit",
     "description": "Maximale Tokens pro Monat"},
    # --- Security ---
    ...
    {"key": "agent_max_escalation_level", "value": "2",
     ...
     "description": "Höchste erlaubte Stufe: 0=Passiv, 1=Aktiv, 2=Vuln, "
     "3=Exploit, 4=Post-Exploit"},
    {"key": "agent_allowed_binaries", "value": (
     ...
     "description": "Komma-getrennte Liste der Binaries die der Agent "
     "ausführen darf"},
    {"key": "agent_blocked_binaries",
     "value": "pip,pip3,apt,apt-get,npm,yarn,brew,cargo,gem",
     "category": "agent", "value_type": "string",
     "label": "Blockierte Binaries",
     "description": "Diese Befehle darf der Agent NIEMALS ausführen "
     "(Paketmanager)"},
    {"key": "agent_tool_install_role", "value": "security_lead",
     "category": "agent", "value_type": "string",
     "label": "Rolle für Tool-Installation",
     "description": "Mindest-Rolle: viewer, analyst, security_lead, "
     "org_admin, system_admin"},
    {"key": "agent_approval_required_level", "value": "3",
     "category": "agent", "value_type": "int",
     "label": "Genehmigung ab Stufe",
     "description": "Ab dieser Eskalationsstufe muss ein Admin genehmigen "
     "(0=immer, 5=nie)"},
    ...
    {"key": "agent_auto_install_enabled", "value": "false",
     ...
     "description": "Wenn deaktiviert: Nur Admin kann Tools über die UI "
     "installieren"},
    {"key": "approval_timeout_seconds", "value": "300",
     "category": "agent", "value_type": "int",
     "label": "Genehmigung-Timeout (s)",
     "description": "Wartezeit auf Genehmigung bevor Stufe 3+ Tool "
     "übersprungen wird"},
    ...
    {"key": "watchdog_webhook_url", "value": "",
     ...
     "description": "POST-Endpoint der bei Kill-Switch-Aktivierung "
     "benachrichtigt wird (leer = deaktiviert)"},
    # --- Backup ---
    ...
    {"key": "nemoclaw_gateway_name", "value": "nemoclaw",
     ...
     "description": "Name des NemoClaw-Gateways für "
     "SSH-Proxy-Verbindungen"},
    ...
    {"key": "nemoclaw_process_limit", "value": "512",
     ...
     "description": "Max. Prozesse in der Sandbox (Fork-Bomb-Schutz). "
     "Siehe: NemoClaw Sandbox Hardening → ulimit -u"},
    {"key": "nemoclaw_read_only_fs", "value": "true",
     "category": "nemoclaw", "value_type": "boolean",
     "label": "Read-Only Dateisystem",
     "description": "Sandbox-Root als Read-Only mounten "
     "(nur /sandbox und /tmp beschreibbar)"},
    {"key": "nemoclaw_tmpfs_size_mb", "value": "64",
     "category": "nemoclaw", "value_type": "int",
     "label": "tmpfs Größe (MB)",
     "description": "Größe des beschreibbaren /tmp Verzeichnisses "
     "in der Sandbox"},
    {"key": "nemoclaw_cap_drop_all", "value": "true",
     "category": "nemoclaw", "value_type": "boolean",
     "label": "Alle Capabilities entfernen",
     "description": "cap_drop=ALL — nur NET_BIND_SERVICE wird "
     "zurückgegeben. Siehe: NemoClaw Security → Capability Drops"},
    {"key": "nemoclaw_no_new_privileges", "value": "true",
     "category": "nemoclaw", "value_type": "boolean",
     "label": "Keine neuen Privilegien",
     "description": "Verhindert Privilege Escalation über "
     "setuid-Binaries. Siehe: NemoClaw Security → "
     "No New Privileges Flag"},
    {"key": "nemoclaw_network_policy", "value": "deny_by_default",
     "category": "nemoclaw", "value_type": "string",
     "label": "Netzwerk-Policy",
     "description": "deny_by_default = nur explizit freigegebene "
     "Endpoints erreichbar. Siehe: NemoClaw → Network Policy"},
    {"key": "nemoclaw_inference_provider", "value": "anthropic",
     "category": "nemoclaw", "value_type": "string",
     "label": "Inference-Provider",
     "description": "LLM-Provider in der Sandbox: anthropic, openai, "
     "nvidia, ollama, compatible"},
    {"key": "nemoclaw_writable_paths", "value": "/sandbox,/tmp",
     "category": "nemoclaw", "value_type": "string",
     "label": "Beschreibbare Pfade",
     "description": "Komma-getrennte Liste der Pfade die in der Sandbox "
     "beschreibbar sind"},
    {"key": "nemoclaw_credential_isolation", "value": "true",
     "category": "nemoclaw", "value_type": "boolean",
     "label": "Credential-Isolation",
     "description": "API-Keys werden nie an die Sandbox übergeben — "
     "nur über Gateway-Proxy. Siehe: NemoClaw Security → "
